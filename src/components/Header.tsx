      ...
      <div className="max-w-7xl mx-auto px-4">
        <div className="flex items-center justify-between h-16">
            ...
            {user && userType === "agency_admin" && <DropdownMenu>
                <DropdownMenuTrigger asChild>
                  <Button variant="ghost" size="lg" className="flex h-14 w-14 hover:scale-110 transition-all duration-300 text-foreground hover:bg-white/20">
                    <Menu strokeWidth={2.5} className="text-white drop-shadow-lg w-[80px] h-[40px]" />
                  </Button>
                </DropdownMenuTrigger>
                <DropdownMenuContent align="start" className="w-64 bg-card z-50 animate-in slide-in-from-top-4 fade-in-0 duration-500 origin-top">
                  <DropdownMenuItem asChild className="hover:bg-accent transition-colors duration-200">
          ...
          <div className="hidden md:flex items-center gap-2 lg:gap-4">
            {/* User Role Indicator */}
            {user && userType && <div className={`flex items-center gap-2 px-4 py-2 rounded-full backdrop-blur-sm border-2 transition-all ${userType === "superadmin" ? "bg-gradient-to-r from-[hsl(200,98%,35%)]/20 to-[hsl(142,76%,30%)]/20 border-[hsl(200,98%,35%)]/60 shadow-lg shadow-[hsl(200,98%,35%)]/30" : "bg-white/10 border-white/30"}`}>
                {userType === "superadmin" ? <Shield className="w-5 h-5 text-[hsl(200,98%,50%)]" fill="currentColor" /> : <span className="relative flex h-3 w-3">
                    <span className="animate-ping absolute inline-flex h-full w-full rounded-full bg-primary/40" style={{
        ...
        background: 'var(--main-gradient)'
            ...
            <nav className="flex flex-col gap-4">
              ...
              {user && userType && <div className={`flex items-center justify-center gap-3 px-5 py-4 mx-4 rounded-lg backdrop-blur-sm border-2 transition-all ${userType === "superadmin" ? "bg-gradient-to-r from-[hsl(200,98%,35%)]/20 to-[hsl(142,76%,30%)]/20 border-[hsl(200,98%,35%)]/60 shadow-lg shadow-[hsl(200,98%,35%)]/30" : "bg-white/10 border-white/30"}`}>
                  {userType === "superadmin" ? <Shield className="w-6 h-6 text-[hsl(200,98%,50%)]" fill="currentColor" /> : <div className="w-3 h-3 rounded-full bg-green-400 animate-pulse" />}
              ...
              {user && userType === "superadmin" && <div className="flex flex-col gap-2 px-4 pb-4 border-b border-white/20">
                  <Link to="/superadmin" className="flex items-center gap-3 px-4 py-3 hover:bg-accent rounded-md transition-colors" onClick={() => setIsMenuOpen(false)}>
                    <BarChart3 className="w-6 h-6" />
                    <span className="font-medium text-lg">Admin Dashboard</span>
                  </Link>
                </div>}
              ...
              {user && userType === "agency_admin" && <div className="flex flex-col gap-2 px-4 pb-4 border-b border-white/20">
              ...
              <div className="px-4">
                <DropdownMenu>
                  ...
                  <DropdownMenuContent align="start" className="w-80 bg-card z-50 animate-in slide-in-from-top-4 fade-in-0 duration-500">
                    <DropdownMenuItem asChild className="hover:bg-accent transition-colors duration-200 cursor-pointer py-4">
