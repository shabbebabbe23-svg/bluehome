...
const CommercialProperties = () => {
  ...
  const filteredProperties = selectedType === "Alla" 
    ? commercialProperties 
    : commercialProperties.filter(p => p.type === selectedType);

  return (
    <div className="min-h-screen dark bg-gradient-to-br from-slate-900 via-slate-800 to-slate-900">
      <Header />
      <main className="pt-20 px-4 max-w-7xl mx-auto">
        <div className="text-center mb-12">
          ...
          <p className="text-slate-300 text-xl">
            Hitta den perfekta lokalen för ditt företag
          </p>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 mb-12">
          {filteredProperties.map((property) => (
            <Card 
              key={property.id} 
              className="overflow-hidden hover:shadow-2xl transition-all duration-300 hover:scale-105 bg-slate-800/50 border-slate-700"
            >
